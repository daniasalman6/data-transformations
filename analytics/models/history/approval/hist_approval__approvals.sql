...
SELECT
    ...
    toLowCardinality(JSONExtractString(_airbyte_data, 'method')) AS method,
    JSONExtractString(_airbyte_data, 'approvers') AS approvers,
    JSONExtractString(_airbyte_data, 'terminal_actions') AS terminal_actions,
    JSONExtractString(_airbyte_data, 'immediate_actions') AS immediate_actions,
    ...
    _airbyte_data
FROM {{ source('airbyte_tables', '_airbyte_raw_approvals') }}
