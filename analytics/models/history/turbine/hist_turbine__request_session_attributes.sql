{{
    config(
        order_by='(update_timestamp, request_id, attribute)',
    )
}}
SELECT
    JSONExtract(_airbyte_data, 'request_id', 'UUID') AS request_id,
    toLowCardinality(JSONExtractString(_airbyte_data, 'attribute')) AS attribute,
    ...
    _airbyte_data
FROM {{ source('airbyte_tables', '_airbyte_raw_request_session_attributes') }}
